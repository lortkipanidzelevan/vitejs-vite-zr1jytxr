...
const WORDLE_ANSWERS: string[] = ["LEBRON", "JORDAN", "KOBE", "CURRY", "MAGIC", "BIRD", "SHAQ", "DURANT", "GIANNIS", "DUNCAN"];
const TODAY_WORD: string = WORDLE_ANSWERS[new Date().getDate() % WORDLE_ANSWERS.length];
...
const ALL_CONNECTION_PLAYERS: string[] = CONNECTIONS_DATA.flatMap(g => g.players).sort(() => Math.random() - 0.5);

const CAREER_PATH_PLAYERS: CareerPlayer[] = [
...
const TODAY_CAREER: CareerPlayer = CAREER_PATH_PLAYERS[new Date().getDate() % CAREER_PATH_PLAYERS.length];
...
const GRID_ROWS: string[] = ["Lakers", "Bulls", "Warriors"];
const GRID_COLS: string[] = ["PG", "SG/SF", "C/PF"];
...
const TODAY_WHO: WhoPlayer = WHO_PLAYERS[new Date().getDate() % WHO_PLAYERS.length];
...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  :root {
    --court: #0a0a0f; --court2: #13131a; --court3: #1c1c27;
    --orange: #FF6B1A; --orange2: #ff8c42; --gold: #FFD700;
    --text: #f0f0f5; --text2: #9090a8;
  ...
  .nav-logo { font-family: 'Bebas Neue', sans-serif; font-size: 28px; color: var(--orange); letter-spacing: 2px; cursor: pointer; display: flex; align-items: center; gap: 8px; }
  .nav-logo span { color: var(--text); }
  .nav-back { background: rgba(255,107,26,0.15); border: 1px solid rgba(255,107,26,0.3); color: var(--orange); padding: 8px 18px; border-radius: 8px; cursor: pointer; font-family: 'DM Sans', sans-serif; font-size: 14px; font-weight: 500; transition: all 0.2s; }
  ...
  .hero::before { content: ''; position: absolute; top: -60px; left: 50%; transform: translateX(-50%); width: 600px; height: 600px; background: radial-gradient(circle, rgba(255,107,26,0.12) 0%, transparent 70%); pointer-events: none; }
  .hero-eyebrow { font-family: 'Barlow Condensed', sans-serif; font-size: 13px; font-weight: 700; letter-spacing: 4px; color: var(--orange); text-transform: uppercase; margin-bottom: 16px; }
  .hero-title { font-family: 'Bebas Neue', sans-serif; font-size: clamp(56px, 10vw, 110px); line-height: 0.9; letter-spacing: 2px; margin-bottom: 20px; }
  .hero-title .accent { color: var(--orange); }
  .hero-sub { font-size: 17px; color: var(--text2); max-width: 480px; margin: 0 auto 48px; line-height: 1.6; font-weight: 300; }
  .games-section { padding: 0 24px 80px; max-width: 1100px; margin: 0 auto; width: 100%; }
  .section-label { font-family: 'Barlow Condensed', sans-serif; font-size: 12px; font-weight: 700; letter-spacing: 4px; color: var(--text2); text-transform: uppercase; margin-bottom: 24px; display: flex; align-items: center; gap: 12px; }
  ...
  .game-card { background: var(--court2); border: 1px solid rgba(255,255,255,0.06); border-radius: var(--radius); padding: 28px 24px; cursor: pointer; transition: all 0.25s ease; position: relative; overflow: hidden; }
  .game-card::before { content: ''; position: absolute; inset: 0; background: linear-gradient(135deg, rgba(255,107,26,0.06) 0%, transparent 60%); opacity: 0; transition: opacity 0.25s; }
  ...
  .game-badge { display: inline-block; margin-top: 16px; font-size: 11px; font-weight: 600; letter-spacing: 2px; text-transform: uppercase; padding: 4px 10px; border-radius: 4px; background: rgba(255,107,26,0.15); color: var(--orange); }
  ...
  .wordle-cell { width: 56px; height: 56px; border: 2px solid rgba(255,255,255,0.12); border-radius: 8px; display: flex; align-items: center; justify-content: center; font-family: 'Barlow Condensed', sans-serif; font-size: 26px; font-weight: 700; transition: all 0.3s; text-transform: uppercase; }
  ...
  .kb-key { min-width: 38px; height: 52px; padding: 0 6px; background: var(--court3); border: none; border-radius: 6px; color: var(--text); font-family: 'DM Sans', sans-serif; font-size: 13px; font-weight: 600; cursor: pointer; transition: all 0.15s; }
  ...
  .conn-item { background: var(--court3); border: 2px solid rgba(255,255,255,0.08); border-radius: 10px; padding: 14px 8px; text-align: center; cursor: pointer; font-size: 13px; font-weight: 500; transition: all 0.2s; line-height: 1.3; }
  .conn-item:hover { border-color: var(--orange); }
  .conn-item.selected { border-color: var(--orange); background: rgba(255,107,26,0.2); }
  .conn-result { border-radius: 10px; padding: 16px; text-align: center; margin-bottom: 8px; font-weight: 600; font-size: 15px; }
  ...
  .career-step { display: flex; align-items: center; gap: 16px; padding: 16px 20px; background: var(--court2); border-left: 3px solid var(--orange); margin-bottom: 4px; border-radius: 0 8px 8px 0; }
  .career-step-num { font-family: 'Bebas Neue', sans-serif; font-size: 22px; color: var(--orange); min-width: 28px; }
  .career-step-team { font-size: 16px; font-weight: 500; }
  .career-step-hidden { background: var(--court3); padding: 14px 20px; border-radius: 8px; margin-bottom: 4px; display: flex; align-items: center; gap: 12px; color: var(--text2); font-size: 14px; }
  .guess-input { width: 100%; background: var(--court2); border: 2px solid rgba(255,255,255,0.1); border-radius: 10px; padding: 14px 18px; color: var(--text); font-family: 'DM Sans', sans-serif; font-size: 16px; margin-bottom: 12px; outline: none; transition: border-color 0.2s; }
  ...
  .attempt-pill { padding: 6px 14px; border-radius: 20px; font-size: 13px; font-weight: 500; }
  ...
  .grid-table th { font-family: 'Barlow Condensed', sans-serif; font-size: 15px; font-weight: 700; letter-spacing: 0.5px; padding: 12px 8px; color: var(--orange); text-align: center; }
  .grid-table .row-label { font-family: 'Barlow Condensed', sans-serif; font-size: 15px; font-weight: 700; color: var(--orange); padding: 8px 16px 8px 0; white-space: nowrap; }
  .grid-cell { background: var(--court2); border: 2px solid rgba(255,255,255,0.07); border-radius: 10px; width: 120px; height: 90px; text-align: center; cursor: pointer; transition: all 0.2s; padding: 8px; vertical-align: middle; }
  ...
  .bingo-header-row { display: grid; grid-template-columns: repeat(5, 1fr); gap: 6px; margin-bottom: 6px; }
  .bingo-col-header { background: var(--orange); color: #fff; text-align: center; font-family: 'Bebas Neue', sans-serif; font-size: 24px; border-radius: 8px; padding: 8px 4px; }
  .bingo-cell { background: var(--court2); border: 1px solid rgba(255,255,255,0.08); border-radius: 8px; padding: 10px 6px; text-align: center; font-size: 11px; font-weight: 500; cursor: pointer; min-height: 64px; display: flex; align-items: center; justify-content: center; transition: all 0.2s; line-height: 1.3; }
  ...
  .player-reveal { background: var(--court2); border-radius: var(--radius); padding: 32px; text-align: center; margin-bottom: 24px; }
  .player-silhouette { font-size: 100px; display: block; margin-bottom: 16px; transition: filter 0.5s; }
  ...
  .clue-item { background: var(--court2); border-radius: 8px; padding: 12px 18px; display: flex; justify-content: space-between; font-size: 14px; }
  ...
  .result-banner { background: var(--court3); border-radius: var(--radius); padding: 24px; text-align: center; margin-bottom: 20px; }
  ...
  .score-badge { background: rgba(255,107,26,0.15); border: 1px solid rgba(255,107,26,0.3); color: var(--orange); border-radius: 8px; padding: 8px 18px; font-family: 'Barlow Condensed', sans-serif; font-size: 20px; font-weight: 700; letter-spacing: 1px; display: inline-block; margin-bottom: 16px; }
...
function WordleGame() {
  ...
  const getLetterState = (word: string, guess: string): string[] => {
    ...
    guess.split("").forEach((l: string, i: number) => {
      if (l === wordArr[i]) { result[i] = "correct"; used[i] = true; }
    });
    guess.split("").forEach((l: string, i: number) => {
      if (result[i] === "correct") return;
      const idx = wordArr.findIndex((wl: string, wi: number) => wl === l && !used[wi]);
  ...
  guesses.forEach(({ guess, states }: GuessRow) => {
    guess.split("").forEach((l: string, i: number) => {
      const s = states[i];
      if (!usedLetters[l] || s === "correct" || (s === "present" && usedLetters[l] === "absent"))
        usedLetters[l] = s;
    });
  });
  ...
  const submit = useCallback(() => {
    ...
    const newGuesses: GuessRow[] = [...guesses, { guess: current.toUpperCase(), states }];
    setGuesses(newGuesses);
    setCurrent("");
    if (current.toUpperCase() === WORD) { setWon(true); setGameOver(true); }
    else if (newGuesses.length >= MAX_GUESSES) { setGameOver(true); }
  }, [current, guesses, WORD]);

  const handleKey = useCallback((key: string) => {
    if (gameOver) return;
    if (key === "ENTER") { submit(); return; }
    if (key === "⌫" || key === "BACKSPACE") { setCurrent((c: string) => c.slice(0, -1)); return; }
    if (/^[A-Z]$/.test(key) && current.length < WORD.length) setCurrent((c: string) => c + key);
  }, [current, gameOver, submit, WORD]);
  ...
  const rows: (GuessRow | null)[] = [...guesses];
  ...
  return (
    <div>
      ...
      <div className="wordle-grid">
        {rows.map((row: GuessRow | null, ri: number) => (
          <div key={ri} className="wordle-row">
            {Array.from({ length: WORD.length }).map((_, ci: number) => {
...
function ConnectionsGame() {
  ...
  const toggle = (p: string) => {
    if (solved.flatMap((s: ConnectionGroup) => s.players).includes(p)) return;
    setSelected((s: string[]) => s.includes(p) ? s.filter((x: string) => x !== p) : s.length < 4 ? [...s, p] : s);
  };

  const submitConn = () => {
    if (selected.length !== 4) return;
    const match = CONNECTIONS_DATA.find((g: ConnectionGroup) =>
      g.players.every((p: string) => selected.includes(p)) && selected.every((p: string) => g.players.includes(p))
    );
    if (match) {
      setSolved((s: ConnectionGroup[]) => [...s, match]);
      setSelected([]);
  ...
  const isSolved = (p: string) => solved.flatMap((s: ConnectionGroup) => s.players).includes(p);
  const unsolved = ALL_CONNECTION_PLAYERS.filter((p: string) => !isSolved(p));

  return (
    <div>
      <div className="game-header"><h1>🔗 CONNECTIONS</h1><p>Find four groups of four basketball players</p></div>
      {solved.map((g: ConnectionGroup) => (
      ...
      {solved.length < 4 && (
        <>
          ...
          <div className="btn-row">
            <button className="btn btn-primary" disabled={selected.length !== 4} onClick={submitConn}>Submit</button>
      ...
      {solved.length === 4 && <div className="result-banner win"><div className="result-title">PERFECT! 🏆</div><div className="result-sub">You found all four groups!</div></div>}
    </div>
  );
}
...
function CareerPathGame() {
  ...
  const submitCareer = () => {
    ...
    setGuess("");
    if (revealed < player.path.length) setRevealed((r: number) => r + 1);
    if (newAttempts.length >= 5) setLost(true);
  };

  return (
    <div>
      <div className="game-header"><h1>🗺️ CAREER PATH</h1><p>Guess the player from their team history</p></div>
      <div className="career-path">
        {player.path.map((team: string, i: number) => (
          i < revealed ? (
      ...
      {attempts.length > 0 && (
        <div className="attempts">
          {attempts.map((a: string, i: number) => <span key={i} className="attempt-pill wrong">✗ {a}</span>)}
        </div>
      )}
      {!won && !lost && (
        <>
          <input className="guess-input" value={guess} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setGuess(e.target.value)}
            onKeyDown={(e: React.KeyboardEvent) => e.key === "Enter" && submitCareer()}
            placeholder="Enter player name..." />
          <div className="btn-row">
            <button className="btn btn-primary" onClick={submitCareer}>Guess</button>
            {revealed < player.path.length && (
              <button className="btn btn-secondary" onClick={() => setRevealed((r: number) => r + 1)}>Reveal next team</button>
            )}
          </div>
        </>
      )}
      {won  && <div className="result-banner win" ><div className="result-title">CORRECT! 🎉</div><div className="result-sub">It was {player.name}!</div></div>}
...
function Box2BoxGame() {
  ...
  const checkAnswer = (row: string, col: string, val: string) => {
    const candidate = GRID_PLAYERS.find((p: GridPlayer) =>
      p.teams.includes(row) && posMap[col].some((pos: string) => p.position === pos) &&
      p.name.toLowerCase() === val.toLowerCase()
    );
    const key = `${row}-${col}`;
    if (candidate) {
      setAnswers((a: Record<string, string>) => ({ ...a, [key]: candidate.name }));
      setScore((s: number) => s + 1);
      setActiveCell(null);
    } else {
      setAnswers((a: Record<string, string>) => ({ ...a, [key]: "❌" }));
      setTimeout(() => setAnswers((a: Record<string, string>) => { const n = { ...a }; delete n[key]; return n; }), 800);
    }
  };

  return (
    <div>
      <div className="game-header"><h1>⬛ BOX2BOX GRID</h1><p>Name a player that fits both the team (row) and position (column)</p></div>
      ...
      <div style={{ overflowX: "auto" }}>
        <table className="grid-table">
          <thead>
            <tr>
              <th></th>
              {GRID_COLS.map((c: string) => <th key={c}>{c}</th>)}
            </tr>
          </thead>
          <tbody>
            {GRID_ROWS.map((row: string) => (
              <tr key={row}>
                <td className="row-label">{row}</td>
                {GRID_COLS.map((col: string) => {
                  const key = `${row}-${col}`;
                      ...
                      {ans ? (
                        ...
                        <div className="grid-cell" style={{ padding: 4 }}>
                          <input
                            ...
                            value={inputVals[key] || ""}
                            onChange={(e: React.ChangeEvent<HTMLInputElement>) => setInputVals((i: Record<string, string>) => ({ ...i, [key]: e.target.value }))}
                            onKeyDown={(e: React.KeyboardEvent) => {
                              if (e.key === "Enter") { checkAnswer(row, col, inputVals[key] || ""); setInputVals((i: Record<string, string>) => ({ ...i, [key]: "" })); }
      ...
      <div style={{ marginTop: 16, fontSize: 13, color: "var(--text2)" }}>Example: Lakers + PG → "Magic"</div>
    </div>
  );
}
...
function BingoGame() {
  ...
  const checkBingo = (m: Set<number>) => {
    ...
    if (lines.some((line: number[]) => line.every((i: number) => m.has(i)))) setBingo(true);
  };
  ...
  return (
    <div>
      <div className="game-header"><h1>🎰 BASKETBALL BINGO</h1><p>Mark the squares that match today's NBA trivia challenge</p></div>
      ...
      <div className="bingo-header-row">
        {["B","I","N","G","O"].map((c: string) => <div key={c} className="bingo-col-header">{c}</div>)}
      </div>
      <div className="bingo-card">
        {BINGO_ITEMS.map((item: string, i: number) => (
          <div key={i} className={`bingo-cell ${i === 11 ? "free" : marked.has(i) ? "marked" : ""}`} onClick={() => toggle(i)}>
            {i === 11 ? "FREE" : item}
          </div>
        ))}
      </div>
      <p style={{ fontSize: 13, color: "var(--text2)" }}>Click squares to mark them. Get 5 in a row to win!</p>
    </div>
  );
}
...
function WhoAreYaGame() {
  ...
  const submitWho = () => {
    ...
    const correct = guess.toLowerCase().includes(last.toLowerCase());
    if (correct) { setWon(true); setRevealed(true); return; }
    const newAttempts = [...attempts, guess.trim()];
    setAttempts(newAttempts);
    setGuess("");
    if (cluesRevealed < clues.length) setCluesRevealed((c: number) => c + 1);
    if (newAttempts.length >= 6) setRevealed(true);
  };

  return (
    <div>
      <div className="game-header"><h1>👤 WHO ARE YA?</h1><p>Guess the NBA player from the clues</p></div>
      ...
      {!won && !revealed && (
        <>
          <input className="guess-input" value={guess}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => setGuess(e.target.value)}
            onKeyDown={(e: React.KeyboardEvent) => e.key === "Enter" && submitWho()}
            placeholder="Enter player name..." />
          <div className="btn-row">
            <button className="btn btn-primary" onClick={submitWho}>Guess</button>
          </div>
        </>
      )}
      {won  && <div className="result-banner win" ><div className="result-title">YOU GOT IT! 🏀</div><div className="result-sub">It was {player.name}!</div></div>}
...
const GAMES: GameDef[] = [
  ...
  { id: "grid",        emoji: "⬛", title: "Box2Box Grid",     desc: "Fill a 3×3 grid with players matching team and position",     badge: "STRATEGY", component: Box2BoxGame },
...
export default function App() {
  ...
  const game = GAMES.find((g: GameDef) => g.id === activeGame);
